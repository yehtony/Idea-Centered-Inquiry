...
export default function ActivityCard({ activity }) {
  ...
  return (
    <div>
      <Item>
        <CardHeader title={activity.ActivityGroup.Activity.title} />
        <CardContent>
          <Typography variant="body2" color="text.secondary">
            {`${formatTimestamp(
              activity.ActivityGroup.Activity.startDate
            )} ~ ${formatTimestamp(activity.ActivityGroup.Activity.endDate)}`}
          </Typography>
        </CardContent>
